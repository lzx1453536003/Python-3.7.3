...
#define SMALL_MEMORY_LIMIT      (64 * 1024 * 1024)      /* 64 MB -- more? */
	...
	*/
#define ARENA_SIZE              (256 << 10)     /* 256KB */ //一个arena中容纳的最大pool个数就是 ARENA_SIZE / POOL_SIZE = 64个。
...
struct pool_header {
	union {
		block *_padding;
		uint count;
	} ref;								/* number of allocated blocks    已分配的 block 数量*/
	block *freeblock;                   /* pool's free list head         */
	struct pool_header *nextpool;       /* next pool of this size class  双向链表，连接相同block大小的pool*/
	struct pool_header *prevpool;       /* previous pool       ""        */
	uint arenaindex;                    /* index into arenas of base adr */
	uint szidx;                         /* block size class index       一個pool分配的block的大小的索引， 一个pool分配的block的大小是一样的 */
	uint nextoffset;                    /* bytes to virgin block         表示freeblock的下一次偏移位置*/
...
struct arena_object {
	 ...
	 * allocated arena.
	 */
	uintptr_t address; //address表示arena的地址

	/* Pool-aligned pointer to the next pool to be carved off. */
	block* pool_address; //pool_address表示arena中未被利用的pool的起始地址；

	/* The number of available pools in the arena:  free pools + never-
	 * allocated pools.
	 */
	uint nfreepools; //nfreepools表示arena中未被利用pool的数量；

	/* The total number of pools in the arena, whether or not available. */
	uint ntotalpools; //ntotalpools表示arena中pool的总数；

	/* Singly-linked list of available pools. */
	struct pool_header* freepools; //freepools指向未被使用的pool的单向链表；
